...
一次性多计算几个neg_item的BPR loss。然后把它们加在一起进行backforward
...
class LightGCN(nn.Module):
    def __init__(self, data_config, args_config, adj_mat):
        ...
        print("拿这个最难的neg_item去和pos_item在每一个维度上进行softmax自适应权重,引入alpha超参数来控制模型倾向,多个negative item进行BPR计算,loss加在一起backforwad......")
    ...
    def forward(self, batch=None,epoch=0):
        ...
        # mf_loss+emb_loss, mf_loss, emb_loss
        # pdb.set_trace()
        loss1,loss2,loss3=0,0,0
        for i_neg_num in range(self.neg_mix_num):
            _neg_gcn_embs = torch.stack([neg_gcn_embs[0][i_neg_num]], dim=1)
            ME_loss, mf_loss, emb_loss=self.create_bpr_loss(user_gcn_emb[user], item_gcn_emb[pos_item], _neg_gcn_embs)
            loss1+=ME_loss
            loss2+=mf_loss
            loss3+=emb_loss
    ...
    def negative_sampling(self, user_gcn_emb, item_gcn_emb, user, neg_candidates, pos_item):
        ...
        # 淦，这TM的绝对是脱了裤子放屁
        # scores2=scores.permute([0, 2, 1])   # [batch_size, n_hops+1, n_negs]
        # scores2[[[i] for i in range(2048)],range(3),indices]=0
        # scores2=scores2.permute([0,2 , 1])   # [batch_size, n_negs, n_hops+1]
        # # x[x==max]=0
        # indices = torch.max(scores2, dim=1)[1].detach()  # torch.Size([2048, 3])
        ...
        for num_neg in range(self.neg_mix_num):
            neg_items_embedding_hardest = neg_items_emb_[[[i] for i in range(batch_size)],range(neg_items_emb_.shape[1]), indices[:,num_neg,:], :]   #   [batch_size, n_hops+1, channel]
            # 采用softmax版本的权重计算
            ...
            n_e_ =  pos_weight * p_e + neg_weight * neg_items_embedding_hardest  # [batch_size, n_hops+1, channel]
            batch_neg_num.append(n_e_)
        return batch_neg_num

...
它的正则损失，不包含neg_item_embedding的正则损失。只包含user_embedding 和 pos_item_embedding的正则损失。
...
class LightGCN(nn.Module):
    ...
    def forward(self, batch=None,epoch=0):
        ...
        return self.create_bpr_loss(user_gcn_emb[user], item_gcn_emb[pos_item])
    ...
    def create_bpr_loss(self, user_gcn_emb, pos_gcn_embs):
        ...
        random_noise = torch.rand_like(pos_e).cuda()  #生成pos_item embedding形状的噪音数据
        neg_e = pos_e + torch.sign(pos_e) * F.normalize(random_noise, dim=-1)

...
此版本的思路是这样的：（用于解决上一版模型权重太靠近pos_item的问题）

给每一个user_embedding，在neg_item 候选集中先选出一个最难的neg_item。

拿这个最难的neg_item去和pos_item在每一个维度上进行softmax的自适应权重。

然后用alpha去控制模型对于正样本分布的倾向性多一点还是少一点
...
class LightGCN(nn.Module):
    def __init__(self, data_config, args_config, adj_mat):
        ...
        self.neg_mix_num = args_config.neg_mix_num
        ...
        print("拿这个最难的neg_item去和pos_item在每一个维度上进行softmax自适应权重,引入alpha超参数来控制模型倾向,在权重分母上mixup多个negative items......")
    ...
    def negative_sampling(self, user_gcn_emb, item_gcn_emb, user, neg_candidates, pos_item):
        ...
        indices = torch.topk(scores, self.neg_mix_num, dim=1, largest=True, sorted=True, out=None)[1].detach()  # [batch_size, neg_mix_num, hops]
        neg_items_emb_ = n_e.permute([0, 2, 1, 3])  # [batch_size, n_hops+1, n_negs, channel]

        pos_score= self.alpha * torch.exp ((s_e * p_e))

        negatives_list=[]
        negative_scores=[]
        for num_neg in range(self.neg_mix_num):
            #  =    #   [batch_size, n_hops+1, channel]
            negatives_list.append(neg_items_emb_[[[i] for i in range(batch_size)],range(neg_items_emb_.shape[1]), indices[:,num_neg,:], :])
            # 采用softmax版本的权重计算
            # neg_scores = torch.exp(s_e *negatives_list[-1])  # [batch_size, n_hops, channel]
            negative_scores.append(torch.exp(s_e *negatives_list[-1]))


        # #--------------------------------------------
        # neg_scores_tensor=torch.rand(2,3,4,5)
        # pos_scores_tensor=torch.rand(2,4,5)
        # negatives=torch.rand(2,3,4,5)
        # total_sum = pos_scores_tensor+neg_scores_tensor.sum(dim=1)   # [batch_size, n_hops, channel]
        # pos_weight = pos_scores_tensor/total_sum
        # neg_weight = neg_scores_tensor/(total_sum.unsqueeze(dim=1))  # [batch_size, numitem ,n_hops, channel]
        # pdb.set_trace()
        # #--------------------------------------------

        neg_scores_tensor = torch.stack(negative_scores,dim=1)  # [batch_size, numitem ,n_hops, channel]
        negatives = torch.stack(negatives_list,dim=1)  # [batch_size, numitem ,n_hops, channel]

        total_sum = pos_score+neg_scores_tensor.sum(dim=1)   # [batch_size, n_hops, channel]
        pos_weight = pos_score/total_sum
        neg_weight = neg_scores_tensor/(total_sum.unsqueeze(dim=1))  # [batch_size, numitem ,n_hops, channel]
        n_e_ = pos_weight * p_e+(neg_weight*negatives).sum(dim=1)
    ...
    def create_bpr_loss(self, user_gcn_emb, pos_gcn_embs, neg_gcn_embs):
        ...
        regularize1 = (torch.norm(user_gcn_emb[:, 1, :]) ** 2
                       ...
                       + torch.norm(neg_gcn_embs[:, :, 1, :]) ** 2) / 2  # take hop=1
        
        regularize2 = (torch.norm(user_gcn_emb[:, 2, :]) ** 2
                       + torch.norm(pos_gcn_embs[:, 2, :]) ** 2
                       + torch.norm(neg_gcn_embs[:, :, 2, :]) ** 2) / 2  # take hop=2
        
        emb_loss = self.decay * (regularize0+regularize1+regularize2) / batch_size

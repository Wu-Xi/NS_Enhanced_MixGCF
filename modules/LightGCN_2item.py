...
此版本用于自适应分配 pos_item 和 hard_neg_item，在每一个维度上的权值。

把自适应权值的思想深入到维度层面。

所有的neg_item_embedding先和user_embedding在每一个维度上单独相乘。

把相应的pos_item_embedding和user_embedding在每一个维度上单独相乘。

两者分别取绝对值再在每一个维度上单独相加作为总的sum。

两者分别在每一个维度上除以上述的sum以此来得到自适应权重。
...
class LightGCN(nn.Module):
    ...
    def negative_sampling(self, user_gcn_emb, item_gcn_emb, user, neg_candidates, pos_item,epoch):
        ...
        neg_scores = torch.abs(s_e.unsqueeze(dim=1) *n_e)  # [batch_size, n_negs, n_hops, channel]
        # total_sum = (s_e * p_e).sum(dim=-1).unsqueeze(dim=1)+neg_scores.sum(dim=-1) # [batch_size, n_negs, n_hops]

        # user_embedding 在某一个维度上是0
        total_sum = torch.abs((s_e * p_e).unsqueeze(dim=1))+neg_scores   # [batch_size, n_negs, n_hops, channel]
        total_sum=total_sum.clamp_(1e-6)

        neg_weight = neg_scores/total_sum     # [batch_size, n_negs, n_hops, channel]
        pos_weight = 1-neg_weight   # [batch_size, n_negs, n_hops, channel]
        n_e_ = pos_weight * p_e.unsqueeze(dim=1) + neg_weight * n_e  # mixing
        if self.last_epoch==epoch:
            pass
        else:
            print(neg_weight[0][0][0],"\n")
            print(neg_weight[0][0][1],"\n")
            print(neg_weight[0][0][2],"\n")
